...
--      assert.is_callable(getmetatable(o).__imports.stage)
    ...
    it("computable", function()
      local o = td.def.computable
      assert.is_table(o)
      assert.is_callable(o)
      assert.is_table(o.__computed)
      assert.is_table(o.__computable)
      ...
      assert.equal('_a', a.typed_name_computable)
      assert.equal('_a', a.untyped_name_computable)
      assert.equal('_a', a.typed_name_computed)
      assert.equal('_a', a.untyped_name_computed)

      assert.is_nil(rawget(a,'typed_name_computable'))
      assert.is_nil(rawget(a,'untyped_name_computable'))
      assert.is_nil(rawget(a,'typed_name_computed'))
      assert.is_nil(rawget(a,'untyped_name_computed'))

      assert.is_nil(rawget(a._,'typed_name_computable'))
      assert.is_nil(rawget(a._,'untyped_name_computable'))
      assert.equal('_a', rawget(a._,'typed_name_computed'))
      assert.equal('_a', rawget(a._,'untyped_name_computed'))
    end)
    it("compute", function()
      local o = td.def.compute
      assert.is_table(o)
      assert.is_callable(o)
      assert.is_table(o.__computed)
      assert.is_table(o.__compute)
      assert.is_function(o.__computed.ok)
      assert.equal(1, #o.__compute)

      local a = o({name='a'})
      assert.equal('ok', a._.ok)
    end)
  end)
end)