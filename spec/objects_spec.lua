...
--    assert.factory(simple)
  ...
  it("new", function()
    ...
    assert.equal({done=true, message='some', created=0}, i)

    i = o('{"done":false, "message":"another", "created":2}')
    assert.equal(i, {done=false, message='another', created=2})
    assert.equal({done=false, message='another', created=2}, i)
  ...
  it("__add/__sub/__concat/__mod/__unm", function()
    ...
    assert.equal(5, o % {})
    assert.equal({_id='66909d26cbade70b6b022b9a', n=3, done=true, message='some', created=0}, o['66909d26cbade70b6b022b9a'])
    assert.equal({_id='66ef5a258aa5f11c0c094b28', n=5}, o['66ef5a258aa5f11c0c094b28'])
  ...
  describe("__div", function()
    describe("auth", function()
      it("defroot", function()
        ...
        assert.equal(nil, auth/{1})
        assert.equal(nil, auth/'')
        assert.equal({_id=oid(id)}, auth/id)
      ...
      it("defitem", function()
        ...
        assert.equal({_id=oid(id)}, item/item)
        assert.equal({_id=oid(id)}, auth/item)
      end)
    end)
